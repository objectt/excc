...
static void on_daily_periodic(nw_periodic *periodic, void *data)
{
    ...
    struct job_request *req_2 = malloc(sizeof(struct job_request));
    memset(req_2, 0, sizeof(struct job_request));
    req_2->callback_fn = load_new_assets_market;
    nw_job_add(job, 1, req_2);
}
