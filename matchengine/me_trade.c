...
int init_trade(void)
{
    ...
    log_stderr("creating %zu markets", settings.market_num);
...
int update_market_dict(market_info_t *market)
{
    market_t *m = market_create(market);
    if (m == NULL)
        return -__LINE__;

    if (dict_add(dict_market, market->name, m)) // TODO distribute market by keys
        return 1;
